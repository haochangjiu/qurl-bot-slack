...
本文面向将 **qurl-bot-slack** 提交至 **Slack 应用目录（App Directory）** 或开启 **公开分发（Manage distribution）** 前的准备与测试。请先完成文中 **第 6 节「上架前测试清单」** 全部项，再进入正式上架流程。
...
| 阶段 | 说明 |
|------|------|
| **阶段 0** | 在 [api.slack.com/apps](https://api.slack.com/apps) **创建应用并完成后台配置**（本文 **第 3 节**），取得凭证并写入 `.env`。 |
| **阶段 A** | 在测试环境完成 **「上架前测试清单」**（本文 **第 6 节**，**必须先做**）。 |
| **阶段 B** | 准备程序以外的材料与合规页面（**第 5 节**）；可先对照 **第 2 节速查表** 查漏。 |
| **阶段 C** | 对照注意事项、官方清单并提交上架（**第 4、7、8 节**）。 |
...
以下为「上架 / 公开分发」时需准备事项的 **一页速查**；与「只跑通代码」不同，还涉及 **域名、证书、合规与 Slack 后台**。详细步骤见 **第 3 节** 起各章。

### 网络与访问

| 准备项 | 说明 |
|--------|------|
| **域名** | 稳定可解析的域名；安装页、OAuth 回调、隐私政策等 URL 建议使用同一主域或合规子域。 |
| **HTTPS / 证书** | 使用 **受信任 CA**（如 Let’s Encrypt）；证书通常配置在 **Nginx / Caddy / 负载均衡**，终止 TLS 后反代到本机 `HTTP_HOST`/`HTTP_PORT`。 |
| **公网可达** | OAuth 回调由用户浏览器访问，须能 **从互联网** 打开 `https://域名/...`（纯内网 IP、自签证书不适合对外上架）。 |
| **（可选）出口 IP** | 若上游 API 对来源 IP 白名单有要求，需预留或文档说明。 |

### Slack 开发者后台

| 准备项 | 说明 |
|--------|------|
| **应用资料** | 名称、简介、图标；上架常需 **截图**、符合 Slack 尺寸的宣传图。 |
| **OAuth** | Redirect URL 与生产 **`SLACK_REDIRECT_URI`** 完全一致（`https://…/slack/oauth_redirect`）。 |
| **权限** | Bot Scopes 与真实功能一致；勿多要。 |
| **Socket Mode** | App-Level Token（`connections:write`）。 |
| **事件与命令** | 已订阅所需 bot 事件；斜杠命令已创建（若必填 Request URL，需为可访问的 `https://…/slack/events`）。 |
| **分发 / 上架** | **Manage distribution** 完成检查；若上 **App Directory**，按向导提交列表信息。 |

### 合规与对外信息

| 准备项 | 说明 |
|--------|------|
| **隐私政策** | 可访问的 **HTTPS** 链接；说明数据收集、**LLM（Anthropic）**、**LayerV API** 等。 |
| **服务条款**（如需） | 部分团队会一并提供。 |
| **支持渠道** | 支持邮箱、工单或帮助页，且能收信、能回复。 |

### 运行环境与密钥

| 准备项 | 说明 |
|--------|------|
| **进程与部署** | 长期运行 `app.py`（或 systemd 等）；默认 OAuth 端口与反代一致。 |
| **环境变量** | `SLACK_*`、`ANTHROPIC_API_KEY`、`LAYERV_*`、`ENCRYPTION_SECRET` 等；生产勿用弱默认密钥。 |
| **数据** | `data/` 下 OAuth 安装目录与 LayerV key 文件路径、备份策略；与隐私政策描述一致。 |

### 第三方与披露

| 准备项 | 说明 |
|--------|------|
| **LayerV** | API 与计费；工作区 Key（`/setkey`）与可选 `LAYERV_API_KEY` 策略。 |
| **Anthropic** | 在隐私政策 / 商店说明中披露 **使用 LLM 理解消息** 及数据范围。 |

### 上架前验证（摘要）

| 准备项 | 说明 |
|--------|------|
| **端到端安装** | 测试工作区完成 `https://域名/slack/install`，OAuth 无报错。 |
| **功能与描述一致** | 商店文案、截图、权限与实际行为一致。 |

---

## 3. Slack 应用注册与后台配置（api.slack.com/apps）

本节描述从 **零创建 Slack 应用** 到与 **qurl-bot-slack** 对齐的完整过程。后台界面可能随 Slack 更新略有差异，以控制台实际文案为准。

### 3.0 测试阶段与上架阶段：HTTP 与 HTTPS

| 阶段 | 建议 |
|------|------|
| **测试 / 开发** | 可使用 **`http://`** 作为 Redirect URL 与安装页（例如 `http://127.0.0.1:8080/slack/oauth_redirect`、`http://公网IP:8080/...`）。**必须与** `.env` 中 **`SLACK_REDIRECT_URI`** 及 Slack 后台 **Redirect URLs** 完全一致。Slack 对非本机的 `http://` 是否允许以控制台为准；若无法保存，可改用 **HTTPS 隧道**（如 ngrok）或仅在可接受的地址上测试。 |
| **正式上架 / 对外分发** | 应使用 **`https://`**（域名 + 有效 TLS 证书），在 Nginx/Caddy 等反代处终止 HTTPS，再转发到本进程的 `HTTP_HOST`/`HTTP_PORT`。上架材料中的安装链接、隐私政策 URL 等也应为 **HTTPS**。 |
...
### 3.4 OAuth & Permissions（OAuth 与权限）
...
3. 本仓库在 `app_home_opened` 时发布欢迎内容，需已订阅该事件（见 3.5 节）。
...
完成以上配置后，在服务器配置 `ANTHROPIC_API_KEY`、`LAYERV_*` 等（见 `.env.example`），启动 `python app.py`，再在浏览器用安装链接测试 OAuth（详见 **第 6 节** 测试清单）。

### 3.9 公开分发与 App Directory（上架入口）

1. 打开 **Settings → Manage distribution**（或 **Distribute App**）。
2. 按 Slack 向导完成 **公开分发** 前置条件（隐私政策、支持联系方式等，与 **第 5 节** 一致）。
...
- **OAuth 多工作区安装**：用户通过浏览器访问 **`http(s)://主机/slack/install`** 安装（**测试**可用 HTTP，**上架后**见 **第 3.0 节** 使用 HTTPS）；回调地址必须与 `.env` 中 `SLACK_REDIRECT_URI` **完全一致**。
- **Socket Mode**：事件通过 Socket Mode 接收，**不要求** 将 Events 的 Request URL 暴露到公网。OAuth 仅用于安装与回调：**测试**时服务可只提供 HTTP；**上架后**对外应对用户提供 **HTTPS**（反代到本进程）。
- **数据**：默认使用本地 **文件**（`SLACK_INSTALLATION_BASE_DIR`、`WORKSPACE_LAYERV_KEYS_PATH`）存储安装信息与 LayerV key；需评估备份、磁盘与多实例部署（多实例时需共享存储或单实例）。
...
| 项目 | 说明 |
|------|------|
| **测试期** | 可用 **`http://`**（本机、内网或 Slack 允许的 `http://` 公网地址），与 Redirect URL 一致即可；无需强制 TLS。 |
| **上架 / 生产** | 使用 **公网 HTTPS**：反向代理（Nginx、Caddy、云 LB 等）将 `https://你的域名` 指向本服务 `HTTP_HOST`/`HTTP_PORT`。 |
| **固定域名** | 正式上架后 Redirect URL、安装链接、隐私政策 URL 建议使用稳定 **HTTPS** 域名；避免长期依赖临时隧道域名。 |
| **TLS 证书** | 生产环境使用有效 HTTPS 证书；Slack OAuth 对证书错误很敏感。 |

### 5.2 Slack 开发者后台（非代码）

应用 **从零创建、逐步点击菜单的配置过程** 见 **第 3 节**；本节从「上架前材料核对」角度列出要点。材料一览见 **第 2 节速查表**。
...
- [ ] 使用 **未安装过该应用** 的测试工作区，通过 **`http://` 或 `https://`**（与配置一致）的 `…/slack/install` 完成安装，无报错（测试常用 `http://`；上架前需用 **HTTPS** 再验一遍）。
...
- [ ] `data/`（安装目录、LayerV key 文件、`oauth_state`）**权限** 合理，备份策略已记录。
...
1. 确认第 6 节 **全部通过** 并留档。  
2. 将生产环境指向 **正式域名与 HTTPS**，并完成第 5 节材料。  
...
- **单点本地文件**：计划多实例前需先解决安装数据与 key 文件的一致性（共享盘或单实例）。
